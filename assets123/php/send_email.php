...
function sendNotificationEmail($to_email, $message, $message_title)
{
    ...
    try {
        ...
        $mail->isSMTP();
        $mail->Host = 'smtp.bitpromarketprime.live';       //email host server
        $mail->SMTPAuth = true;
        $mail->Username = 'info@bitpromarketprime.live';       //email username
        $mail->Password = 'info@bitpromarketprime.live';       //email password
        $mail->SMTPSecure = 'ssl';
        $mail->Port = 465;

        // Recipients
        $mail->setFrom('info@bitpromarketprime.live', 'BitProMarketPrime');       //(email sender details to be displayed)
