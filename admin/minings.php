...
$query = "SELECT um.id, u.username, m.name AS miner_name, um.amount, um.start_date, um.end_date, um.status, um.currency, um.duration, um.used_duration
        FROM mining_history um
...
$userMiners = $result->fetch_all(MYSQLI_ASSOC);
		...
		<div class="app-content pt-3 p-md-3 p-lg-4">
			<div class="container-xl">
				...
				<div class="row g-3 mb-4 align-items-center justify-content-between">
					<div class="col-auto">
						<h1 class="app-page-title mb-0">Active Mining Records</h1>
					</div>
				</div>
									...
									<table class="table app-table-hover mb-0 text-left">
										<thead>
											<tr>
												...
												<th class="cell">Miner</th>
												<th class="cell">Amount</th>
												<th class="cell">Days Mined</th>
												<th class="cell">Days Remaining</th>
												<th class="cell">Status</th>
												<th class="cell">Action</th> <!-- Cancel Mining -->
											</tr>
										</thead>
										<tbody>
											<?php if (!empty($userMiners)) : ?>
												<?php $i = count($userMiners); ?>
												<?php foreach ($userMiners as $record) : ?>
													<tr>
														<td class="cell"><?= $i--; ?></td>
														<td class="cell"><?= htmlspecialchars($record['username']); ?></td>
														<td class="cell"><?= htmlspecialchars($record['miner_name']); ?></td>
														<td class="cell"><?= number_format($record['amount'], 2) . ' ' . $record['currency'] ?></td>
														<td class="cell"><?= htmlspecialchars($record['used_duration']); ?></td>
														<td class="cell"><?= htmlspecialchars($record['duration']); ?></td>
														<td class="cell">
														...
														<td class="cell">
															<?php if ($record['status'] == 'active') : ?>
																<form method="POST" action="assets/php/cancel_mining.php" class="d-inline">
																	<input type="hidden" name="record_id" value="<?= $record['id']; ?>">
																	<button type="submit" name="action" value="cancel" class="btn btn-danger btn-sm">Cancel Mining</button>
																</form>
															<?php else : ?>
											...
											<?php else : ?>
												<tr>
													<td class="cell" colspan="8">No mining records found.</td>
												</tr>
											<?php endif; ?>
										</tbody>
									</table>
